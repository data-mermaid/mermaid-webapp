...
3. `nag-suppressions.ts` contains **stack-level** suppressions for findings that have been triaged — each one is tagged with a reason and status.
...
```bash
cd iac && npx cdk synth --quiet 2>&1 | grep '^\[Error'
```
