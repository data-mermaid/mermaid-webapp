...
const ImageAnnotationPopup = ({
    ...
    const areAnyClassifierGuesses = selectedPoint.annotations.filter((annotation) => annotation.is_machine_created,).length

    const isSelectedPointUnclassified = selectedPoint.annotations[0].ba_gr === unclassifiedGuid
    const areAllPointsConfirmed = dataToReview.points.every((point) => point.annotations[0]?.is_confirmed,)

    const confirmPoint = useCallback((pointId) => {
        const updatedPoints = dataToReview.points.map((point) => {
            ...
            return {
                ...point, annotations: updatedAnnotations,
            }
        })
    ...
    return (<>
        {!!areAnyClassifierGuesses && (<div aria-labelledby="table-label">
            <PointPopupSectionHeader>
                <span>
