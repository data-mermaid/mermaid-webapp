...
import SubmittedBeltInvertInfoTable from './SubmittedBeltInvertInfoTable'
import SubmittedBeltInvertObservationTable from './SubmittedBeltInvertObservationTable'
  ...
  const handleMoveToCollect = () => {
    ...
    databaseSwitchboardInstance
      .moveToCollect({
        projectId,
        submittedRecordId,
        sampleUnitMethod: 'beltinverttransectmethods',
      })
      .then(({ id }) => {
        toast.success(...getToastArguments(submittedRecordMovedToCollectingMessage))
        navigate(`${ensureTrailingSlash(currentProjectPath)}collecting/macroinvertebrate/${id}`)
      })
      .catch((error) => {
        handleHttpResponseError({
          error,
...
export default SubmittedBeltInvert
