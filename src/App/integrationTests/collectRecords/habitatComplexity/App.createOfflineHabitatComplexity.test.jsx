...
const saveHabitatComplexityRecord = async (user) => {
  ...
  await user.type(intervalSizeInput, '7')
  expect(intervalSizeInput).toHaveValue(7)
...
describe('Offline', () => {
  test('New Habitat Complexity save success shows saved input values, toast, and navigates to edit fishbelt page for new record', async () => {
    ...
    await saveHabitatComplexityRecord(user)
    ...
    await waitFor(() => {
      ...
      expect(screen.getByTestId('interval-size-input')).toHaveValue(7)
  ...
  test('New Habitat Complexity save failure shows toast message with edits persisting', async () => {
    ...
    expect(screen.getByTestId('habitatcomplexity-page-title'))
