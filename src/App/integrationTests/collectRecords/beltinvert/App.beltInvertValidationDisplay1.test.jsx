...
test('Macroinvertebrate validations will show the all warnings when there are multiple warnings and no errors', async () => {
  ...
  mockMermaidApiAllSuccessful.use(
    ...
    http.post(`${apiBaseUrl}/pull/`, () => {
      ...
      const collectRecordWithValidation = {
        ...mockBeltInvertCollectRecords[0],
        data: {
          ...mockBeltInvertCollectRecords[0].data,
          sample_event: {
            ...mockBeltInvertCollectRecords[0].data.sample_event,
            site: '',
            management: '',
            sample_date: '',
          },
          beltinvert_transect: {
            ...mockBeltInvertCollectRecords[0].data.beltinvert_transect,
            depth: '',
            label: '',
            width: '',
            size_bin: '',
...
test('Macroinvertebrate validations will show passed input validations', async () => {
  const { dexiePerUserDataInstance, dexieCurrentUserInstance } = getMockDexieInstancesAllSuccess()
