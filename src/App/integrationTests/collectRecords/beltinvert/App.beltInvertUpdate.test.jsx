...
describe('Offline', () => {
  ...
  test.skip('(TODO - TEST TECH DEBT) Edit belt invert save stores properly formatted belt invert observations in dexie', async () => {
    ...
    const newInvertNameInput = screen.getAllByLabelText('Macroinvertebrate Name')[3]
    ...
    await user.type(newInvertNameInput, 'neb')

    const invertNameList = screen.getAllByRole('listbox')[3]

    const nebriusOption = screen.getByRole('option', {
      name: 'Nebrius',
    })

    await user.selectOptions(invertNameList, nebriusOption)
    ...
    const newObservation = updatedCollectRecord.data.obs_belt_inverts[3]

    expect(newObservation.invert_attribute).toEqual('018c6b47-9e6f-456d-8db2-ce1c91e8e1c4')
    expect(newObservation.count).toEqual(88)
    expect(newObservation.size).toEqual(37.5)
  })
  test.skip('(TODO - TEST TECH DEBT) Edit belt invert save stores properly formatted belt invert observations in dexie for 50+ observation size input', async () => {
    const { dexiePerUserDataInstance, dexieCurrentUserInstance } = getMockDexieInstancesAllSuccess()
    ...
    const newObservation = savedCollectRecord.data.obs_belt_inverts[3]

    expect(newObservation.size).toEqual(50367)
  })
  test.skip('(TODO - TEST TECH DEBT) Edit belt invert save failure shows toast message with new edits persisting', async () => {
    const { dexiePerUserDataInstance, dexieCurrentUserInstance } = getMockDexieInstancesAllSuccess()
