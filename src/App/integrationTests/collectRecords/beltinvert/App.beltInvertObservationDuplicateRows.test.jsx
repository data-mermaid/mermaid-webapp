...
test('Macroinvertebrate observations: pressing tab while the notes cell is focused on the last row duplicates the row', async () => {
  ...
  const allRows = within(observationsTableBeforeTabKey).getAllByRole('row')
  const lastObservationRow = allRows[allRows.length - 1]
  const lastNotesCell = within(lastObservationRow).getByRole('button', { name: /edit notes/i })

  // userEvent doesnt work as expected for tab
  fireEvent.keyDown(lastNotesCell, { key: 'Tab', code: 'Tab' })
  ...
  expect(
    within(observationsTableAfterTab).getAllByDisplayValue('Test Proposed Invert Attribute').length,
  ).toEqual(2)

  const newFishNameInput = within(observationsTableAfterTab).getAllByDisplayValue(
    'Test Proposed Invert Attribute',
  )[1]

  expect(newFishNameInput).toHaveFocus()
})

test('Macroinvertebrate observations: enter key adds a new empty row below row where key pressed', async () => {
  const { dexiePerUserDataInstance, dexieCurrentUserInstance } = getMockDexieInstancesAllSuccess()
