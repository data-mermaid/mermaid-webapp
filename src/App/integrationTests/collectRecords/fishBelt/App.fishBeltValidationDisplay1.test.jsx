...
test('Validating an empty collect record, and then editing an input with errors shows the errors until the save button is pressed. Validations show when the validation button is clicked again.', async () => {
  ...
  expect(within(screen.getByTestId('width')).queryByText('validation_messages.required')).not.toBeInTheDocument()
  expect(within(screen.getByTestId('size-bin')).queryByText('validation_messages.required')).not.toBeInTheDocument()
